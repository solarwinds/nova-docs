(function () {
  function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

  function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

  function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

  function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([["status-chart-status-example-module"], {
    /***/
    "13ZT":
    /*!**************************************************************************************************************************************************!*\
      !*** ./node_modules/raw-loader/dist/cjs.js!./examples/components/demo/development/status/waterfall-simple/chart-waterfall-simple.component.html ***!
      \**************************************************************************************************************************************************/

    /*! exports provided: default */

    /***/
    function ZT(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony default export */


      __webpack_exports__["default"] = "<div class=\"chart\">\n    <nui-chart class=\"d-flex\" [chart]=\"gridChart\"></nui-chart>\n</div>\n";
      /***/
    },

    /***/
    "6biZ":
    /*!************************************************************************************!*\
      !*** ./examples/components/demo/development/status/chart-status-example.module.ts ***!
      \************************************************************************************/

    /*! exports provided: ChartStatusExampleModule */

    /***/
    function biZ(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "ChartStatusExampleModule", function () {
        return ChartStatusExampleModule;
      });
      /* harmony import */


      var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! tslib */
      "mrSG");
      /* harmony import */


      var _angular_cdk_drag_drop__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! @angular/cdk/drag-drop */
      "5+WD");
      /* harmony import */


      var _angular_core__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! @angular/core */
      "fXoL");
      /* harmony import */


      var _angular_router__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      /*! @angular/router */
      "tyNb");
      /* harmony import */


      var _nova_ui_bits__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(
      /*! @nova-ui/bits */
      "Rr1A");
      /* harmony import */


      var _nova_ui_charts__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(
      /*! @nova-ui/charts */
      "gKry");
      /* harmony import */


      var _common_demo_common_module__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(
      /*! ../../common/demo-common.module */
      "s1Ul");
      /* harmony import */


      var _status_test_chart_status_test_component__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(
      /*! ./status-test/chart-status-test.component */
      "HsLF");
      /* harmony import */


      var _waterfall_simple_chart_waterfall_simple_component__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(
      /*! ./waterfall-simple/chart-waterfall-simple.component */
      "TOoM");
      /* harmony import */


      var _waterfall_test_chart_waterfall_test_component__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(
      /*! ./waterfall-test/chart-waterfall-test.component */
      "q6nO");

      var collectionRoutes = [{
        path: "",
        component: _status_test_chart_status_test_component__WEBPACK_IMPORTED_MODULE_7__["ChartStatusTestComponent"]
      }, {
        path: "waterfall",
        component: _waterfall_test_chart_waterfall_test_component__WEBPACK_IMPORTED_MODULE_9__["ChartWaterfallTestComponent"]
      }, {
        path: "waterfall-simple",
        component: _waterfall_simple_chart_waterfall_simple_component__WEBPACK_IMPORTED_MODULE_8__["ChartWaterfallSimpleComponent"]
      }];

      var ChartStatusExampleModule = function ChartStatusExampleModule() {
        _classCallCheck(this, ChartStatusExampleModule);
      };

      ChartStatusExampleModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([Object(_angular_core__WEBPACK_IMPORTED_MODULE_2__["NgModule"])({
        declarations: [_status_test_chart_status_test_component__WEBPACK_IMPORTED_MODULE_7__["ChartStatusTestComponent"], _waterfall_simple_chart_waterfall_simple_component__WEBPACK_IMPORTED_MODULE_8__["ChartWaterfallSimpleComponent"], _waterfall_test_chart_waterfall_test_component__WEBPACK_IMPORTED_MODULE_9__["ChartWaterfallTestComponent"]],
        imports: [_angular_cdk_drag_drop__WEBPACK_IMPORTED_MODULE_1__["DragDropModule"], _nova_ui_bits__WEBPACK_IMPORTED_MODULE_4__["NuiIconModule"], _common_demo_common_module__WEBPACK_IMPORTED_MODULE_6__["DemoCommonModule"], _nova_ui_charts__WEBPACK_IMPORTED_MODULE_5__["NuiChartsModule"], _angular_router__WEBPACK_IMPORTED_MODULE_3__["RouterModule"].forChild(collectionRoutes)]
      })], ChartStatusExampleModule);
      /***/
    },

    /***/
    "HsLF":
    /*!************************************************************************************************!*\
      !*** ./examples/components/demo/development/status/status-test/chart-status-test.component.ts ***!
      \************************************************************************************************/

    /*! exports provided: ChartStatusTestComponent */

    /***/
    function HsLF(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "ChartStatusTestComponent", function () {
        return ChartStatusTestComponent;
      });
      /* harmony import */


      var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! tslib */
      "mrSG");
      /* harmony import */


      var _raw_loader_chart_status_test_component_html__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! raw-loader!./chart-status-test.component.html */
      "I7Qu");
      /* harmony import */


      var _angular_core__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! @angular/core */
      "fXoL");
      /* harmony import */


      var _nova_ui_bits__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      /*! @nova-ui/bits */
      "Rr1A");
      /* harmony import */


      var _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(
      /*! @nova-ui/charts */
      "gKry");
      /* harmony import */


      var lodash_range__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(
      /*! lodash/range */
      "WjpJ");
      /* harmony import */


      var lodash_range__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(lodash_range__WEBPACK_IMPORTED_MODULE_5__);
      /* harmony import */


      var moment_moment__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(
      /*! moment/moment */
      "wd/R");
      /* harmony import */


      var moment_moment__WEBPACK_IMPORTED_MODULE_6___default = /*#__PURE__*/__webpack_require__.n(moment_moment__WEBPACK_IMPORTED_MODULE_6__);

      var Status;

      (function (Status) {
        Status["Up"] = "up";
        Status["Warning"] = "warning";
        Status["Critical"] = "critical";
      })(Status || (Status = {}));

      var ChartStatusTestComponent = /*#__PURE__*/function () {
        function ChartStatusTestComponent(iconService) {
          var _this = this,
              _nova_ui_charts__WEBP;

          _classCallCheck(this, ChartStatusTestComponent);

          this.iconService = iconService;
          this.chartThreshold = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BarStatusGridConfig"]()));
          this.chartBand = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BarStatusGridConfig"]()));

          var getStatusMarker = function getStatusMarker(status) {
            return new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["SvgMarker"](_this.iconService.getStatusIcon(status));
          };

          this.customMarkers = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["MappedValueProvider"]((_nova_ui_charts__WEBP = {}, _defineProperty(_nova_ui_charts__WEBP, Status.Up, getStatusMarker(Status.Up)), _defineProperty(_nova_ui_charts__WEBP, Status.Warning, getStatusMarker(Status.Warning)), _defineProperty(_nova_ui_charts__WEBP, Status.Critical, getStatusMarker(Status.Critical)), _nova_ui_charts__WEBP));
        }

        _createClass(ChartStatusTestComponent, [{
          key: "ngAfterViewInit",
          value: function ngAfterViewInit() {
            var _this2 = this;

            this.chartThreshold.updateDimensions();
            this.chartBand.updateDimensions();
            var bandScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BandScale"]();
            bandScale.fixDomain(_nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["StatusAccessors"].STATUS_DOMAIN);
            var timeIntervalScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["TimeIntervalScale"](moment_moment__WEBPACK_IMPORTED_MODULE_6___default.a.duration(100, "hours"));
            timeIntervalScale.fixDomain([moment_moment__WEBPACK_IMPORTED_MODULE_6___default()().toDate(), moment_moment__WEBPACK_IMPORTED_MODULE_6___default()().add(1000, "hours").toDate()]);
            var timeScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["TimeScale"](); // This formats label in interaction label plugin

            timeScale.formatters.labelFormatter = function (d) {
              return moment_moment__WEBPACK_IMPORTED_MODULE_6___default()(d).format("LL");
            };

            this.thresholdScales = {
              y: bandScale,
              x: timeScale
            };
            this.bandScales = {
              y: bandScale,
              x: timeIntervalScale
            };
            var renderer = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BarRenderer"]();
            var accessors = Object(_nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["statusAccessors"])(new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["MappedValueProvider"](getStatusValueMap())); // Thickness accessor should be used to specify which status corresponds to a thin bar or thick

            accessors.data.thickness = function (data) {
              return data.status === Status.Up ? _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BarRenderer"].THIN : _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["BarRenderer"].THICK;
            }; // Here we assemble the complete chart series.


            var thresholdSeriesSet = getData({
              numberOfSeries: 10,
              isRandom: true
            }).map(function (d) {
              return Object.assign(Object.assign({}, d), {
                accessors: accessors,
                renderer: renderer,
                scales: _this2.thresholdScales
              });
            }); // Here we assemble the complete chart series.

            var bandSeriesSet = getData({
              numberOfSeries: 1,
              isRandom: false
            }).map(function (d) {
              return Object.assign(Object.assign({}, d), {
                accessors: accessors,
                renderer: renderer,
                scales: _this2.thresholdScales
              });
            });
            this.chartThreshold.update(thresholdSeriesSet);
            this.chartThreshold.setSeriesStates(thresholdSeriesSet.map(function (series) {
              return {
                seriesId: series.id,
                state: _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["RenderState"].deemphasized
              };
            }));
            this.chartBand.update(bandSeriesSet);
          }
        }]);

        return ChartStatusTestComponent;
      }();

      ChartStatusTestComponent.ctorParameters = function () {
        return [{
          type: _nova_ui_bits__WEBPACK_IMPORTED_MODULE_3__["IconService"]
        }];
      };

      ChartStatusTestComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([Object(_angular_core__WEBPACK_IMPORTED_MODULE_2__["Component"])({
        selector: "nui-status-chart-test-example",
        template: _raw_loader_chart_status_test_component_html__WEBPACK_IMPORTED_MODULE_1__["default"]
      }), Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [_nova_ui_bits__WEBPACK_IMPORTED_MODULE_3__["IconService"]])], ChartStatusTestComponent);

      function getStatusValueMap() {
        var _ref;

        return _ref = {}, _defineProperty(_ref, Status.Up, _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["CHART_PALETTE_CS_S"][4]), _defineProperty(_ref, Status.Warning, _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["CHART_PALETTE_CS_S"][2]), _defineProperty(_ref, Status.Critical, _nova_ui_charts__WEBPACK_IMPORTED_MODULE_4__["CHART_PALETTE_CS_S"][1]), _ref;
      }
      /* Chart data */


      function getData() {
        var config = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {
          numberOfSeries: 1,
          isRandom: true
        };

        var getDate = function getDate(hours) {
          return moment_moment__WEBPACK_IMPORTED_MODULE_6___default()().add({
            hours: hours
          }).toDate();
        };

        var generateThresholdData = function generateThresholdData(isRandom) {
          return function () {
            var nextStart = 0;
            var thresholds = [];

            while (nextStart < 1000) {
              var nextFinish = nextStart + (isRandom ? Math.random() * 100 : 100);

              if (nextFinish >= 1000) {
                nextFinish = 1000;
              }

              var statusSeed = Math.random();
              thresholds.push({
                start: nextStart,
                end: nextFinish,
                status: statusSeed > 0.5 ? Status.Up : statusSeed > 0.2 ? Status.Warning : Status.Critical
              });
              nextStart = nextFinish;
            }

            return thresholds;
          };
        };

        var seriesData = lodash_range__WEBPACK_IMPORTED_MODULE_5___default()(config.numberOfSeries);
        return seriesData.map(generateThresholdData(config.isRandom)).map(function (series, i) {
          return {
            id: "series-" + i,
            name: "Series " + i,
            data: series.map(function (d) {
              return {
                value: d.end - d.start,
                status: d.status,
                start: getDate(d.start),
                end: getDate(d.end)
              };
            })
          };
        });
      }
      /***/

    },

    /***/
    "I7Qu":
    /*!****************************************************************************************************************************************!*\
      !*** ./node_modules/raw-loader/dist/cjs.js!./examples/components/demo/development/status/status-test/chart-status-test.component.html ***!
      \****************************************************************************************************************************************/

    /*! exports provided: default */

    /***/
    function I7Qu(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony default export */


      __webpack_exports__["default"] = "<p class=\"nui-text-page\">\n    Threshold section example\n</p>\n\n<div class=\"d-flex flex-column\">\n    <nui-chart class=\"d-flex\" [chart]=\"chartThreshold\" class=\"p-2\"></nui-chart>\n</div>\n\n<p class=\"nui-text-page\">\n    Band (Band'ish) scales on both scales\n</p>\n\n<div class=\"d-flex flex-column\">\n    <nui-chart class=\"d-flex\" [chart]=\"chartBand\" class=\"p-2\"></nui-chart>\n</div>\n";
      /***/
    },

    /***/
    "Ii80":
    /*!********************************************************************************************************!*\
      !*** ./examples/components/demo/development/status/waterfall-test/chart-waterfall-test.component.less ***!
      \********************************************************************************************************/

    /*! exports provided: default */

    /***/
    function Ii80(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony default export */


      __webpack_exports__["default"] = "/** draggable items states */\n.nui-dnd-hover-state {\n  background: #f2f2f2;\n}\n.nui-dnd-raised-state {\n  z-index: 1;\n  box-sizing: border-box;\n  box-shadow: 0 2px 10px 0 var(--nui-shadow-color, rgba(17, 17, 17, 0.3));\n}\n.nui-dnd-preview {\n  border: 1px #d9d9d9 solid;\n  background: #f2f2f2;\n  z-index: 1;\n  box-sizing: border-box;\n  box-shadow: 0 2px 10px 0 var(--nui-shadow-color, rgba(17, 17, 17, 0.3));\n}\n.nui-dnd-dropzone {\n  /** dropzone states */\n  /** other customziations */\n}\n.nui-dnd-dropzone.nui-dnd-dropzone--active {\n  border: 4px dashed #999 !important;\n}\n.nui-dnd-dropzone.nui-dnd-dropzone--drop-allowed {\n  cursor: move;\n  border: 4px solid #00c4d2 !important;\n  background-color: rgba(0, 196, 210, 0.2);\n}\n.nui-dnd-dropzone.nui-dnd-dropzone--drop-not-allowed {\n  cursor: move;\n  border: 4px solid #dd2c00 !important;\n  background-color: rgba(221, 44, 0, 0.2);\n}\n.nui-dnd-dropzone .centered-overlay,\n.nui-dnd-dropzone .nui-icon-item:before {\n  position: absolute;\n  top: 50%;\n  left: 50%;\n  transform: translate(-50%, -50%);\n}\n.nui-dnd-dropzone .nui-icon-item:before {\n  border-radius: 50%;\n  content: \"\";\n  display: block;\n  height: 40px;\n  width: 40px;\n  z-index: -1;\n}\n.nui-dnd-dropzone .drop-circle-permanent .nui-icon-item:before {\n  background-color: #fff;\n}\n.nui-dnd-dropzone .drop-circle-permanent .nui-icon-item svg polygon {\n  fill: #111;\n}\n.nui-dnd-dropzone .drop-circle-allowed .nui-icon-item:before {\n  background-color: #00c4d2;\n}\n.nui-dnd-dropzone .drop-circle-allowed .nui-icon-item svg polygon {\n  -webkit-clip-path: circle(5px);\n          clip-path: circle(5px);\n  fill: #fff;\n}\n.nui-dnd-dropzone .drop-circle-not-allowed .nui-icon-item:before {\n  background-color: #dd2c00;\n}\n.nui-dnd-dropzone .drop-circle-not-allowed .nui-icon-item svg path {\n  -webkit-clip-path: circle(5px);\n          clip-path: circle(5px);\n  fill: #fff;\n}\n.waterfall-chart-list-item:hover {\n  background-color: var(--nui-color-selected,rgba(0, 196, 210, 0.1));\n}\n.waterfall-chart-list-item {\n  background-color: var(--nui-color-bg-content,#fff);\n}\n\n/*# sourceMappingURL=data:application/json;base64, */";
      /***/
    },

    /***/
    "TOoM":
    /*!**********************************************************************************************************!*\
      !*** ./examples/components/demo/development/status/waterfall-simple/chart-waterfall-simple.component.ts ***!
      \**********************************************************************************************************/

    /*! exports provided: ChartWaterfallSimpleComponent */

    /***/
    function TOoM(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "ChartWaterfallSimpleComponent", function () {
        return ChartWaterfallSimpleComponent;
      });
      /* harmony import */


      var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! tslib */
      "mrSG");
      /* harmony import */


      var _raw_loader_chart_waterfall_simple_component_html__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! raw-loader!./chart-waterfall-simple.component.html */
      "13ZT");
      /* harmony import */


      var _angular_core__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! @angular/core */
      "fXoL");
      /* harmony import */


      var _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      /*! @nova-ui/charts */
      "gKry");
      /**
       * This is here just to test a prototype of angular component, that will use new chart core
       *
       * @ignore
       */


      var ChartWaterfallSimpleComponent = /*#__PURE__*/function () {
        function ChartWaterfallSimpleComponent() {
          _classCallCheck(this, ChartWaterfallSimpleComponent);

          this.palette = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["ChartPalette"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["MappedValueProvider"]({
            "connect": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["CHART_PALETTE_CS1"][0],
            "dns": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["CHART_PALETTE_CS1"][1],
            "send": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["CHART_PALETTE_CS1"][2],
            "ttfb": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["CHART_PALETTE_CS1"][3],
            "cdownload": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["CHART_PALETTE_CS1"][4]
          }));
          this.gridChart = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["BarHorizontalGridConfig"]()));
          this.listItems = [{
            category: "Category 1",
            data: [{
              type: "connect",
              start: 0,
              end: 22
            }, {
              type: "dns",
              start: 22,
              end: 39
            }, {
              type: "send",
              start: 39,
              end: 59
            }, {
              type: "ttfb",
              start: 59,
              end: 109
            }, {
              type: "cdownload",
              start: 109,
              end: 178
            }]
          }, {
            category: "Category 2",
            data: [{
              type: "connect",
              start: 0,
              end: 22
            }, {
              type: "dns",
              start: 22,
              end: 39
            }, {
              type: "send",
              start: 39,
              end: 59
            }, {
              type: "ttfb",
              start: 59,
              end: 109
            }, {
              type: "cdownload",
              start: 109,
              end: 788
            }]
          }, {
            category: "Category 3",
            data: [{
              type: "connect",
              start: 178,
              end: 222
            }, {
              type: "dns",
              start: 222,
              end: 239
            }, {
              type: "send",
              start: 239,
              end: 259
            }, {
              type: "ttfb",
              start: 259,
              end: 309
            }, {
              type: "cdownload",
              start: 309,
              end: 578
            }]
          }, {
            category: "Category 4",
            data: [{
              type: "connect",
              start: 578,
              end: 590
            }, {
              type: "dns",
              start: 590,
              end: 799
            }, {
              type: "send",
              start: 799,
              end: 888
            }, {
              type: "ttfb",
              start: 888,
              end: 900
            }, {
              type: "cdownload",
              start: 900,
              end: 990
            }]
          }];
        }

        _createClass(ChartWaterfallSimpleComponent, [{
          key: "ngAfterViewInit",
          value: function ngAfterViewInit() {
            var _this3 = this;

            var bandScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["BandScale"]();
            bandScale.padding(0.5);
            var linearScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["LinearScale"]();
            this.scales = {
              y: bandScale,
              x: linearScale
            };

            this.scales.x.formatters.tick = function (value) {
              return "".concat(Number(value / 1000).toFixed(1), "s");
            };

            var renderer = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["BarRenderer"]({
              highlightStrategy: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["BarHighlightStrategy"]("x")
            });
            var accessors = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_3__["HorizontalBarAccessors"]();

            accessors.data.color = function (d) {
              return _this3.palette.standardColors.get(d.type);
            };

            var categories = [];
            var seriesSet = [];
            this.listItems.forEach(function (item, i) {
              categories.push(item.category);
              seriesSet.push({
                id: "series-".concat(i),
                name: "Series ".concat(i),
                data: item.data.map(function (d) {
                  return _defineProperty({
                    value: d.end - d.start,
                    category: item.category,
                    type: d.type
                  }, "__bar", {
                    start: d.start,
                    end: d.end
                  });
                }),
                accessors: accessors,
                scales: _this3.scales,
                renderer: renderer
              });
            });
            bandScale.fixDomain(categories.reverse());
            this.gridChart.update(seriesSet);
          }
        }]);

        return ChartWaterfallSimpleComponent;
      }();

      ChartWaterfallSimpleComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([Object(_angular_core__WEBPACK_IMPORTED_MODULE_2__["Component"])({
        selector: "nui-chart-waterfall-simple",
        template: _raw_loader_chart_waterfall_simple_component_html__WEBPACK_IMPORTED_MODULE_1__["default"]
      })], ChartWaterfallSimpleComponent);
      /***/
    },

    /***/
    "o9I1":
    /*!**********************************************************************************************************************************************!*\
      !*** ./node_modules/raw-loader/dist/cjs.js!./examples/components/demo/development/status/waterfall-test/chart-waterfall-test.component.html ***!
      \**********************************************************************************************************************************************/

    /*! exports provided: default */

    /***/
    function o9I1(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony default export */


      __webpack_exports__["default"] = "<div>\n    <div cdkDropList class=\"example-list\" (cdkDropListDropped)=\"drop($event)\">\n        <div *ngFor=\"let item of listItems; let i = index; let first = first\" class=\"d-flex flex-row align-items-center waterfall-chart-list-item\" cdkDrag>\n            <nui-icon icon=\"{{item.icon}}\" class=\"p-2\"></nui-icon>\n            <div style=\"overflow: hidden;\" [style.width.px]=\"200\" class=\"p-2\">\n                <span>{{item.url}}</span>\n            </div>\n            <div class=\"d-flex\">\n                <div class=\"d-flex h-100\" *ngIf=\"first\" #gridChartPlaceholder></div>\n                <nui-chart class=\"d-flex\" [chart]=\"item.chart\"></nui-chart>\n            </div>\n        </div>\n    </div>\n</div>\n\n<ng-template #templatePortalGrid>\n    <nui-chart class=\"d-flex\" [chart]=\"gridChart\"></nui-chart>\n</ng-template>\n";
      /***/
    },

    /***/
    "q6nO":
    /*!******************************************************************************************************!*\
      !*** ./examples/components/demo/development/status/waterfall-test/chart-waterfall-test.component.ts ***!
      \******************************************************************************************************/

    /*! exports provided: ChartWaterfallTestComponent */

    /***/
    function q6nO(module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "ChartWaterfallTestComponent", function () {
        return ChartWaterfallTestComponent;
      });
      /* harmony import */


      var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! tslib */
      "mrSG");
      /* harmony import */


      var _raw_loader_chart_waterfall_test_component_html__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! raw-loader!./chart-waterfall-test.component.html */
      "o9I1");
      /* harmony import */


      var _chart_waterfall_test_component_less__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! ./chart-waterfall-test.component.less */
      "Ii80");
      /* harmony import */


      var _angular_cdk_drag_drop__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      /*! @angular/cdk/drag-drop */
      "5+WD");
      /* harmony import */


      var _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(
      /*! @angular/cdk/overlay */
      "rDax");
      /* harmony import */


      var _angular_cdk_portal__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(
      /*! @angular/cdk/portal */
      "+rOU");
      /* harmony import */


      var _angular_core__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(
      /*! @angular/core */
      "fXoL");
      /* harmony import */


      var _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(
      /*! @nova-ui/charts */
      "gKry");
      /**
       * This is here just to test a prototype of angular component, that will use new chart core
       *
       * @ignore
       */


      var ChartWaterfallTestComponent = /*#__PURE__*/function () {
        function ChartWaterfallTestComponent(overlay, overlayPositionBuilder, _viewContainerRef, scrollStrategyOptions) {
          _classCallCheck(this, ChartWaterfallTestComponent);

          this.overlay = overlay;
          this.overlayPositionBuilder = overlayPositionBuilder;
          this._viewContainerRef = _viewContainerRef;
          this.scrollStrategyOptions = scrollStrategyOptions; // TODO: 1. overlay for grid. 2. top axis 3. popup on hover example

          this.palette = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["ChartPalette"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["MappedValueProvider"]({
            "connect": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["CHART_PALETTE_CS1"][0],
            "dns": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["CHART_PALETTE_CS1"][1],
            "send": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["CHART_PALETTE_CS1"][2],
            "ttfb": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["CHART_PALETTE_CS1"][3],
            "cdownload": _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["CHART_PALETTE_CS1"][4]
          }));
          this.gridChart = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["XYGrid"]());
          this.listItems = [{
            url: "http://www.google.com",
            size: 924,
            icon: "xml-file",
            chart: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BarStatusGridConfig"]({
              showBottomAxis: false
            }))),
            data: [{
              type: "connect",
              start: 0,
              end: 22
            }, {
              type: "dns",
              start: 22,
              end: 39
            }, {
              type: "send",
              start: 39,
              end: 59
            }, {
              type: "ttfb",
              start: 59,
              end: 109
            }, {
              type: "cdownload",
              start: 109,
              end: 178
            }]
          }, {
            url: "http://www2.google.com",
            size: 924,
            icon: "xml-file",
            chart: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BarStatusGridConfig"]({
              showBottomAxis: false
            }))),
            data: [{
              type: "connect",
              start: 0,
              end: 22
            }, {
              type: "dns",
              start: 22,
              end: 39
            }, {
              type: "send",
              start: 39,
              end: 59
            }, {
              type: "ttfb",
              start: 59,
              end: 109
            }, {
              type: "cdownload",
              start: 109,
              end: 788
            }]
          }, {
            url: "http://www.google.com/cat.png",
            size: 3333,
            icon: "image",
            chart: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BarStatusGridConfig"]({
              showBottomAxis: false
            }))),
            data: [{
              type: "connect",
              start: 178,
              end: 222
            }, {
              type: "dns",
              start: 222,
              end: 239
            }, {
              type: "send",
              start: 239,
              end: 259
            }, {
              type: "ttfb",
              start: 259,
              end: 309
            }, {
              type: "cdownload",
              start: 309,
              end: 578
            }]
          }, {
            url: "http://www.google.com/revenge.png",
            size: 3333,
            icon: "image",
            chart: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["Chart"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["XYGrid"](new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BarStatusGridConfig"]({
              showBottomAxis: false
            }))),
            data: [{
              type: "connect",
              start: 578,
              end: 590
            }, {
              type: "dns",
              start: 590,
              end: 799
            }, {
              type: "send",
              start: 799,
              end: 888
            }, {
              type: "ttfb",
              start: 888,
              end: 900
            }, {
              type: "cdownload",
              start: 900,
              end: 990
            }]
          }];
        }

        _createClass(ChartWaterfallTestComponent, [{
          key: "ngOnInit",
          value: function ngOnInit() {
            var _this4 = this;

            this.gridChart.addPlugin(new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["ZoomPlugin"]());
            this.gridChart.getEventBus().getStream(_nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["SET_DOMAIN_EVENT"]).subscribe(function (event) {
              var payload = event.data;

              _this4.listItems.forEach(function (item) {
                _this4.scales.x.fixDomain(payload[_this4.scales.x.id]);

                item.chart.updateDimensions();
              });
            });
          }
        }, {
          key: "ngAfterViewInit",
          value: function ngAfterViewInit() {
            var _this5 = this;

            this.templatePortal = new _angular_cdk_portal__WEBPACK_IMPORTED_MODULE_5__["TemplatePortal"](this.templatePortalGrid, this._viewContainerRef);
            var positions = [{
              overlayX: "start",
              overlayY: "top",
              originX: "start",
              originY: "top"
            }];
            this.positionStrategy = this.overlayPositionBuilder.flexibleConnectedTo(this.gridChartPlaceholder).withPositions(positions).withFlexibleDimensions(true).withPush(true);
            var overlayRef = this.overlay.create({
              positionStrategy: this.positionStrategy,
              scrollStrategy: this.scrollStrategyOptions.close()
            });
            var bandScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BandScale"]();
            var linearScale = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["LinearScale"]();
            linearScale.fixDomain([0, 1000]);
            bandScale.fixDomain(["cat1"]);
            this.scales = {
              y: bandScale,
              x: linearScale
            };

            this.scales.x.formatters.tick = function (value) {
              return "".concat(Number(value / 1000).toFixed(1), "s");
            };

            var renderer = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["BarRenderer"]();
            var accessors = new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["HorizontalBarAccessors"]();

            accessors.data.color = function (d) {
              return _this5.palette.standardColors.get(d.type);
            };

            var commonWidth = 0;
            this.listItems.forEach(function (item) {
              var seriesSet = [{
                id: "series-1",
                name: "Series 1",
                data: item.data.map(function (d) {
                  return _defineProperty({
                    value: d.end - d.start,
                    category: "cat1",
                    type: d.type
                  }, "__bar", {
                    start: d.start,
                    end: d.end
                  });
                }),
                accessors: accessors,
                scales: _this5.scales,
                renderer: renderer
              }];
              item.chart.update(seriesSet);
              item.chart.getEventBus().getStream(_nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["HIGHLIGHT_DATA_POINT_EVENT"]).subscribe(console.log);
              item.chart.getEventBus().getStream(_nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["SELECT_DATA_POINT_EVENT"]).subscribe(console.log);
              commonWidth = item.chart.getGrid().config().dimension.width(); // TODO: executed n times
            }); // Handle grid

            var config = this.gridChart.getGrid().config();
            config.dimension.height(this.listItems.length * 32);
            config.dimension.width(commonWidth);
            config.dimension.autoHeight = false;
            config.axis.left.visible = false;
            config.axis.left.gridTicks = false;
            config.axis.bottom.gridTicks = true;
            this.gridChart.update([{
              id: "i am grid",
              name: "i am grid",
              data: [],
              accessors: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["NoopAccessors"](),
              scales: this.scales,
              renderer: new _nova_ui_charts__WEBPACK_IMPORTED_MODULE_7__["NoopRenderer"]()
            }]);
            this.gridChart.updateDimensions(); // this.templatePortalGrid.elementRef.nativeElement.offsetHeight = config.dimension.height;

            overlayRef.attach(this.templatePortal);
          }
        }, {
          key: "drop",
          value: function drop(event) {
            Object(_angular_cdk_drag_drop__WEBPACK_IMPORTED_MODULE_3__["moveItemInArray"])(this.listItems, event.previousIndex, event.currentIndex);
          }
        }]);

        return ChartWaterfallTestComponent;
      }();

      ChartWaterfallTestComponent.ctorParameters = function () {
        return [{
          type: _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["Overlay"]
        }, {
          type: _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["OverlayPositionBuilder"]
        }, {
          type: _angular_core__WEBPACK_IMPORTED_MODULE_6__["ViewContainerRef"]
        }, {
          type: _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["ScrollStrategyOptions"]
        }];
      };

      ChartWaterfallTestComponent.propDecorators = {
        templatePortalGrid: [{
          type: _angular_core__WEBPACK_IMPORTED_MODULE_6__["ViewChild"],
          args: ["templatePortalGrid"]
        }],
        gridChartPlaceholder: [{
          type: _angular_core__WEBPACK_IMPORTED_MODULE_6__["ViewChild"],
          args: ["gridChartPlaceholder"]
        }]
      };
      ChartWaterfallTestComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([Object(_angular_core__WEBPACK_IMPORTED_MODULE_6__["Component"])({
        selector: "nui-chart-waterfall-test",
        template: _raw_loader_chart_waterfall_test_component_html__WEBPACK_IMPORTED_MODULE_1__["default"],
        styles: [_chart_waterfall_test_component_less__WEBPACK_IMPORTED_MODULE_2__["default"]]
      }), Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [_angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["Overlay"], _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["OverlayPositionBuilder"], _angular_core__WEBPACK_IMPORTED_MODULE_6__["ViewContainerRef"], _angular_cdk_overlay__WEBPACK_IMPORTED_MODULE_4__["ScrollStrategyOptions"]])], ChartWaterfallTestComponent);
      /***/
    }
  }]);
})();
//# sourceMappingURL=status-chart-status-example-module-es5.js.map