...
<!--[if gt IE 8]><!--> <html class="no-js xui" <% if(!deferLoad) { %>ng-app="docsApp" <% } %> lang="en" ng-controller="DocsController"> <!--<![endif]-->
<head>
  <meta charset="utf-8">
        ...
        <div class="container">
          <% if (image) { %>
            <% if (imageLink) { %>
              <a href="<%= imageLink %>">
            <% } %>
              <img class="pull-left xui-sw-logo" src="<%= image %>">
            <% if (imageLink) { %>
              </a>
            <% } %>
          <% } %>
          <% if (title) {%>
            ...
            <span class="xui-version"> _XUI_VERSION_FULL_ </span>
          <% } %>
          <ul class="nav">
