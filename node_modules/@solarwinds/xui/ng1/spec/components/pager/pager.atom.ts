...
export class PagerAtom extends Atom {
    ...
    public setItemsPerPage = (itemsPerPage: number): promise.Promise<void> => {
        super.getElement().all(by.id("pageNumberMenu1")).get(0).click();
        return super.getElement().element(by.css('[aria-labelledby="pageNumberMenu1"] .xui-pager__item-per-page'))
            .element(by.xpath(".//button[.=" + itemsPerPage + "]")).click();
    };

    public itemsPerPageSelection = (): promise.Promise<string> => {
        return super.getElement().all(by.id("pageNumberMenu1")).get(0).getText().then((test) => {
    ...
    public isPageLinkVisible = (pageNumber: number): promise.Promise<boolean> => {
    ...
    public isEllipsisLinkDisplayed = (index: number): promise.Promise<boolean> => {
        return super.getElement()
    ...
    public async isEllipsisHasDropupClass(): Promise<boolean> {
        await this.ellipsisLinkClick(0);
        const dropdownElement = await super.getElement()
            .all(by.css(".xui-pager__items *[uib-dropdown]")).get(0);

        return await Atom.hasClass(dropdownElement, "dropup");
    }

    public async isItemsDispHasDropupClass(): Promise<boolean> {
        const itemsShownElem = await super.getElement()
            .all(by.className("xui-pager__items-shown")).get(0);
        await itemsShownElem.click();

        const dropdownElement = await super.getElement()
                .all(by.css(".xui-pager__info *[uib-dropdown]")).get(0);
        return await Atom.hasClass(dropdownElement, "dropup");
    };
